...
public class XformMappedTest {
    ...
    public void mapInOneBatch() {
        ...
        assertArrayEquals(ints,
                          seq.map(Fn1.identity()).toMutList().toArray());

        assertArrayEquals(new Integer[] { 2,3,4,5,6,7,8,9,10 },
                          seq.map(plusOne).toMutList().toArray());

        assertArrayEquals(new Integer[] { 0,1,2,3,4,5,6,7,8 },
                          seq.map(minusOne).toMutList().toArray());

        assertArrayEquals(ints,
                          seq.map(plusOne).map(minusOne).toMutList().toArray());

        assertArrayEquals(ints,
                          seq.map(minusOne).map(plusOne).toMutList().toArray());

        assertArrayEquals(new Integer[] { 3,4,5,6,7,8,9,10,11 },
                          seq.map(plusOne).map(plusOne).toMutList().toArray());
