 ...
 */
public class KeyValuePair<K,V> extends Tuple2<K,V> implements Map.Entry<K,V>,
        UnmodMap.UnEntry<K,V>, Serializable {
    // For serializable.  Make sure to change whenever internal data format changes.
    private static final long serialVersionUID = 20160827174100L;

    /** Key/Value Constructor */
    public KeyValuePair(K k, V v) { super(k, v); }

    /** Map.Entry Constructor */
    public KeyValuePair(Map.Entry<K,V> entry) { super(entry.getKey(), entry.getValue()); }

    /** Public static Key/Value factory method */
    public static <K,V> KeyValuePair<K,V> of(K a, V b) { return new KeyValuePair<>(a, b); }

    /** Public static Map.Entry factory method */
    public static <K,V> KeyValuePair<K,V> of(Map.Entry<K,V> entry) { return new KeyValuePair<>(entry); }
    ...
    @Override @Deprecated public V setValue(V value) {
        throw new UnsupportedOperationException("KeyValuePair is immutable");
    }
    ...
    private static class SerializationProxy<K,V> implements Serializable {
        private final K k;
        private final V v;
        SerializationProxy(KeyValuePair<K,V> kv) { k = kv._1; v = kv._2; }

        private static final long serialVersionUID = 20160827174100L;

        private Object readResolve() { return new KeyValuePair<>(k, v); }
    }
