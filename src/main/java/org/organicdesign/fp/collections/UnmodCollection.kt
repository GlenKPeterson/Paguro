 ...
 *
 * UnmodCollection is an unmodifiable version of [java.util.Collection]
 * which formalizes the return type of Collections.unmodifiableCollection() and Map.values().
 */
@PurelyImplements("java.util.Collection")
interface UnmodCollection<E> : Collection<E>, UnmodIterable<E>, Sized {
    ...
    fun add(element: E): Boolean = throw UnsupportedOperationException("Modification attempted")

    /** Not allowed - this is supposed to be unmodifiable  */
    @JvmDefault
    @Deprecated("Not allowed - this is supposed to be unmodifiable")
    fun addAll(elements: Collection<E>): Boolean = throw UnsupportedOperationException("Modification attempted")
    ...
    @JvmDefault
    override fun containsAll(elements: Collection<E>): Boolean = containsAll(this, elements)
    ...
    @Deprecated("Not allowed - this is supposed to be unmodifiable")
    @JvmDefault
    fun remove(element: E): Boolean = throw UnsupportedOperationException("Modification attempted")

    /** Not allowed - this is supposed to be unmodifiable  */
    @Deprecated("Not allowed - this is supposed to be unmodifiable")
    @JvmDefault
    fun removeAll(elements: Collection<E>): Boolean = throw UnsupportedOperationException("Modification attempted")

    /** Not allowed - this is supposed to be unmodifiable  */
    @Deprecated("Not allowed - this is supposed to be unmodifiable")
    @JvmDefault
    fun removeIf(filter: (E) -> Boolean): Boolean = throw UnsupportedOperationException("Modification attempted")

    /** Not allowed - this is supposed to be unmodifiable  */
    @Deprecated("Not allowed - this is supposed to be unmodifiable")
    @JvmDefault
    fun retainAll(elements: Collection<E>): Boolean = throw UnsupportedOperationException("Modification attempted")
     ...
     */
    @JvmDefault
    override fun spliterator(): Spliterator<E> = Spliterators.spliterator(this, SIZED)
     ...
     */
    @JvmDefault
    fun toArray(): Array<out Any?> = UnmodIterable.toArray(this, arrayOfNulls<Any>(size), size())
    ...
    fun <T> toArray(elements: Array<T>): Array<out T> =
            UnmodIterable.toArray(this as Iterable<T>, elements, size())
//    /**
//     * This method goes against Josh Bloch's Item 25: "Prefer Lists to Arrays", but is provided for backwards
//     * compatibility.  If you really need an array, consider using the somewhat type-safe version of this method
//     * instead, but read the caveats first.
//     */
//    @JvmDefault
//    fun toArray(): Array<out Any?> = toArray(arrayOfNulls(size))
//
//    /**
//     * This method goes against Josh Bloch's Item 25: "Prefer Lists to Arrays", but is provided for backwards
...
//            @Suppress("UNCHECKED_CAST")
//            elems = java.lang.reflect.Array.newInstance(elems.javaClass.getComponentType(), size) as Array<T>
////            elems = arrayOfNulls(size)
//        }
//        val iter = iterator()
//        for (i in 0 until size) {
//            @Suppress("UNCHECKED_CAST")
//            elems[i] = iter.next() as T
//        }
//        if (size < elems.size) {
...
//    }
    companion object {
        ...
        // O(this size PLUS that size), whereas looping through both would be
        // O(this size TIMES that size).
        fun <T> containsAll(coll:Collection<T>, elements: Collection<T>): Boolean =
                when {
                    elements.isEmpty() -> true
                    coll.isEmpty()     -> false
                    else               -> coll.toMutableSet().containsAll(elements)
                }
