...
// OK, this is a dumb comment, or maybe it makes some sense, but since B-trees grow from the top, it's probably
// really hard to implement.  I'll probably delete this after checking it in once.
//
// Question: Would it help to use a smaller branching factor at the top of the tree for fewer comparisons, maybe
//     Make the top level 4-way branch, the next 8-way branch, then 16-way, and finally 32-way branch at all lower
//     levels?  That could save storage space for smaller trees and maybe minimize the amount of data copied on each
//     modification.  Hmm... it would get you down the right branch faster, but it would do so by increasing the depth
//     of the tree.  If the number of nodes being changed is the limiting factor, than level-attenuated-branching would
//     actually slow it down.  If the size of the nodes is the limiting factor, this could speed it up.
//
// Say there are 2l branches per level, where l is the level, with l=1 being the top level.
// That makes a very neat little tree to start with just 2 spaces.
//
// l = 1, size = 4
//                    1, 2, 3, 4
// Comparisons: Retrieve top-level bucket
//              idx < size // Is this a legal index?
//              then array lookup
// Total retrievals: 1
// Total comparisons: 1
// Total copy-to-cache: 1
// Number of dwords rewritten for prepend/append/insert-at-middle: 6
// (assuming internal nodes have 2 + num pointers dwords)
//
// l = 2, size = 32
//                  8, 8, 8, 8
//               ,-'  /    \  `--.
...
// Comparisons: Retrieve top-level bucket
//              idx < l1.size,     // Is this a legal index (top-level check only)?
//              idx < (l1.size/2),    // Do we search from the left or the right
//              Retrieve l1.bucket1
//              idx < l1.bucket1.size
//              Retrieve l1.bucket2
//              idx < l1.bucket2.size
...
// Total retrievals: 3
// Total comparisons: 4
// Number of dwords rewritten for prepend/append/insert-at-middle: 16
//
//  l = 3, size = 512
//                                                                  128, 128, 128, 128
//                               ,----------------------------------' ,--'       '-.  `----------------------------.
...
//              idx < l1.bucket1.size
//              Retrieve l1.bucket2
//              idx < (l2.size / 2) // Do we search from the left or the right
//              Retrieve l2.bucket1
//              idx < l2.bucket1.size
//              Retrieve l2.bucket2
//              idx < l2.bucket2.size
...
// Total retrievals: 7
// Total comparisons: 8
// Number of dwords rewritten for prepend/append/insert-at-middle: 34
//
// l = 4, size = 16384
//                                                                                           4096, 4096, 4096, 4096
//                                                                                          /          \     \     `-...
...
//     ,---------'                                               `----------.        ,-----------'
// ,--. ,-. ,-. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--. ,--.
// 1-32 -64 -96 -128 -160 -192 -224 -256 -288 -320 -352 -384 -416 -448 -480 -512 -544 -576 -608 -640 -672 -704 -736 ...
// Comparisons: Retrieve top-level bucket
//              idx < l1.size,     // Is this a legal index (top-level check only)?
//              idx < (l1.size/2), // Do we search from the left or the right
//              Retrieve l1.bucket1
...
//              Retrieve l2.bucket4
//              idx < l2.bucket4.size
//              idx < (l3.size / 2) // Do we search from the left or the right
//              Retrieve l3.bucket1
//              idx < l3.bucket1.size
//              Retrieve l3.bucket2
//              idx < l3.bucket2.size
...
// Number of dwords rewritten for prepend/append/insert-at-middle: 68
